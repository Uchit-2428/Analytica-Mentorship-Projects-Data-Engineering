...
@st.cache()
def load_data(nrows):
    data = pd.read_csv("database.csv", nrows=nrows)
    return data

database = load_data(190000)
...
def main():
    ...
    if choice == 'Home':
        ...
        if st.checkbox('Best month'):
            ...
            results = database.groupby('Month').sum()
        ...
        if st.checkbox('Best Pair'):
            ...
            df = database[database['Order ID'].duplicated(keep=False)]
        ...
        if st.checkbox('Most Sold'):
            ...
            product_group = database.groupby('Product')
